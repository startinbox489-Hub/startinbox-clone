'use server';

import ConfigService from '@/lib/config';
import { NextResponse } from 'next/server';

export async function POST(req: Request) {
	try {
		const body = await req.json();
		const response = await fetch(
			`${await ConfigService.get('BASE_URL')}/api/v1/auth/signup/google`,
			{
				headers: {
					'Content-Type': 'application/json',
				},
				body: JSON.stringify(body),
				method: 'POST',
				credentials: 'include',
			},
		);
		let data;
		if (response.headers.get('content-type') !== 'application/json') {
			data = await response.text();
			return NextResponse.json(
				{
					message: 'Internal Server Error. Contact Us with the code #gsin001',
					status: 'error',
					user: null,
				},
				{ status: 500 },
			);
		} else {
			data = await response.json();
		}

		if (response.status === 422) {
			console.log('data: ', data);
			let message: string = data.data.message;
			if (data.data?.msg.startsWith('Value error')) {
				message = data.data?.msg.substring(12);
			}
			return NextResponse.json(
				{
					message: message.replace(/_/g, ' ') || data.message,
					data: data.data,
					user: null,
				},
				{ status: response.status },
			);
		}

		if ([409, 417].includes(response.status)) {
			return NextResponse.json(
				{ message: data.message, user: null },
				{ status: response.status },
			);
		}

		if (response.status > 499) {
			return NextResponse.json(
				{
					message: 'Internal Server Error. Contact Us with the code #gsin001',
					status: 'error',
					user: null,
				},
				{ status: response.status },
			);
		}

		// Store tokens in httpOnly cookies
		const access_token = data?.data?.token?.access_token;
		const refresh_token = response.headers.get('x-refresh-token');

		const headers = new Headers();
		headers.append(
			'Set-Cookie',
			`access_token=${access_token}; Path=/; HttpOnly; Secure; SameSite=Lax`,
		);
		headers.append(
			'Set-Cookie',
			`refresh_token=${refresh_token}; Path=/; HttpOnly; Secure; SameSite=Lax`,
		);

		return NextResponse.json(
			{ message: data.message, user: data.data.user },
			{ status: response.status, headers },
		);
	} catch (error) {
		console.error('proxy Error signing up user via google:', error);
		return NextResponse.json(
			{ message: 'Internal Server Error', user: null },
			{ status: 500 },
		);
	}
}
